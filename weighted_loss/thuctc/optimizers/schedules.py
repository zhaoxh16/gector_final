...
class LinearWarmupLinearDecay(LearningRateSchedule):
    ...
    def __init__(self, max_lr, warmup_steps, train_steps,
                 init_lr=0.0, min_lr=0.0, summary=True):
        super(LinearWarmupLinearDecay, self).__init__()

        if init_lr <= 0:
            if warmup_steps > 0:
                init_lr = max_lr / float(warmup_steps)
            else:
                init_lr = 0.0
        elif init_lr > max_lr:
            raise ValueError(
                "The maximum learning rate: {} must be higher than "
                "the initial learning rate: {}".format(max_lr, init_lr)
            )
    ...
    def __call__(self, step):
        if step <= self._warmup_steps:
            lr_step = self._max_lr - self._init_lr
            lr_step /= self._warmup_steps
            lr = self._init_lr + lr_step * step
        else:
            if self._warmup_steps > 0:
                all_steps = float(self._train_steps - self._warmup_steps)
            else:
                all_steps = float(self._train_steps)

            decay_rate = float(self._train_steps - step) / all_steps
            lr = self._min_lr + (self._max_lr - self._min_lr) * decay_rate

...
def build_input_fn(filenames, mode, params):
    def train_input_fn():
        ...
        dataset = dataset.map(
            lambda x, y: (
                {
                    "source": x["source"],
                    "source_mask": x["source_mask"]
                },
                {
                    "label": y["label"],
                    "label_mask": y['label_mask'],
                    "correct_mask": (tf.math.equal(y['label'], b'$KEEP') & tf.cast(y['label_mask'], bool)),
                    "incorrect_mask": (tf.math.logical_not(tf.math.equal(y['label'], b'$KEEP')) & tf.cast(y['label_mask'], bool))
                }
    ...
    def eval_input_fn():
        ...
        dataset = dataset.map(
            lambda x, y:(
                tf.concat([[tf.constant(params.bos)], x, [tf.constant(params.eos)]], axis=0),
                tf.concat([[tf.constant(params.label_pad)], y, [tf.constant(params.label_pad)]], axis=0)),
                num_parallel_calls=tf.data.experimental.AUTOTUNE)

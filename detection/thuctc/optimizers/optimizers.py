...
from thuctc.optimizers.schedules import LearningRateSchedule
...
class AdamWOptimizer(Optimizer):
    ...
    def __init__(self, learning_rate=0.01, beta_1=0.9, beta_2=0.999,
                 epsilon=1e-8, weight_decay=1e-2, name="AdamW", **kwargs):
    ...
    def apply_gradients(self, grads_and_vars):
        ...
        for grad, var in zip(grads, var_list):
            ...
            # weight decay
            if var.dtype == torch.float32:
                var.data.mul_(1 - lr * self._weight_decay)
            else:
                fp32_var = var.data.float()
                fp32_var.data.mul_(1 - lr * self._weight_decay)
...
class BertAdamOptimizer(Optimizer):
    ...
    def __init__(self, learning_rate=0.01, beta_1=0.9, beta_2=0.999,
                 epsilon=1e-8, weight_decay=1e-2, name="BertAdam", **kwargs):
        super(BertAdamOptimizer, self).__init__(name, **kwargs)
        self._learning_rate = learning_rate
        self._beta_1 = beta_1
        self._beta_2 = beta_2
        self._epsilon = epsilon
    ...
    def apply_gradients(self, grads_and_vars):
        ...
        for grad, var in zip(grads, var_list):
            ...
            m.mul_(beta_1).add_(grad, alpha=1.0 - beta_1)
            ...
            if var.dtype == torch.float32:
                var.data.addcdiv_(m, denom, value=-step_size)
                var.data.mul_(1.0 -lr * self._weight_decay)
            else:
                fp32_var = var.data.float()
                fp32_var.addcdiv_(m, denom, value=-step_size)
                fp32_var.mul_(1.0 - lr * self._weight_decay)
                var.data.copy_(fp32_var.data)
